...
%%         io:format("param identifier LIST: ~p~n",[List]),
      ...
      {text, _, _}=T  -> find_text_template(T);
      {text, _}=T  -> find_text_template(T);
      {string, _, _}=T  -> find_text_template(T);
      {string, _}=T  -> find_text_template(T);
   ...
   lists:foldl(
      fun(E1, Acc) ->
         case E1 of
            ...
            _O ->
%%               io:format("extract_refs OTHER: ~p~n",[_O]),
               Acc
         end
      end,
      [],
